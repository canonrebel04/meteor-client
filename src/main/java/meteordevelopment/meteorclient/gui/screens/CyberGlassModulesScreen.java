...
import meteordevelopment.meteorclient.gui.cyberglass.WCyberGlassModuleCard;
...
public class CyberGlassModulesScreen extends TabScreen {
    private WCategoryController controller;
    private long lastFavoritesSignature = Long.MIN_VALUE;

    public CyberGlassModulesScreen(GuiTheme theme) {
    ...
    public void initWidgets() {
        ...
        help.add(theme.label("Right click - Inline module settings"));
    }
    ...
    protected void createSearchW(WContainer w, String text) {
        ...
        if (!modules.isEmpty()) {
            ...
            for (Module module : modules) {
                ...
                section.add(new WCyberGlassModuleCard(module)).expandX();
                count++;
            }
        }
    ...
    protected WWindow createSearch(WContainer c) {
        return createSearch((WCategoryController) c);
    }

    protected WWindow createSearch(WCategoryController owner) {
        WWindow w = theme.window("Search");
        w.id = "cyber-glass-search";
    ...
    protected boolean createFavoritesW(WWindow w) {
        ...
        for (Module module : modules) {
            w.add(new WCyberGlassModuleCard(module)).expandX();
        }
    ...
    public void tick() {
        ...
        long signature = favoritesSignature();
        ...
        if (controller.tickSearchRefresh()) {
            invalidate();
        }
    }

    private long favoritesSignature() {
        long signature = 0;
        int count = 0;
        ...
        for (Module module : Modules.get().getAll()) {
            ...
            long h = module.name.hashCode();
            signature ^= (h * 0x9E3779B97F4A7C15L);
        }

        signature ^= ((long) count) << 32;
        return signature;
    }
    ...
    protected WWindow createCategory(WContainer c, Category category, List<Module> moduleList) {
        ...
        w.id = "cyber-glass-" + category.name;
        w.padding = 0;
        w.spacing = 0;
    ...
    private class WCategoryController extends WContainer {
        ...
        private boolean tickSearchRefresh() {
            if (searchText == null || searchResults == null) return false;
            ...
            searchRefreshTimer++;
            if (searchRefreshTimer < 20) return false;
            searchRefreshTimer = 0;

            searchResults.clear();
            createSearchW(searchResults, query);
            return true;
        }
