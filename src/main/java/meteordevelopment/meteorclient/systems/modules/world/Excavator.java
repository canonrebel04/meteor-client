...
public class Excavator extends Module {
    ...
    public void onDeactivate() {
        BaritoneUtils.removeAllSelections(BaritoneUtils.getSelectionManager(baritone));
    ...
    private void selectCorners() {
        ...
        if (status == Status.SEL_START) {
            start = new BetterBlockPos(result.getBlockPos());
            status = Status.SEL_END;
            if (logSelection.get()) {
                info("Start corner set: (%d, %d, %d)".formatted(start.getX(), start.getY(), start.getZ()));
            }
        } else if (status == Status.SEL_END) {
            end = new BetterBlockPos(result.getBlockPos());
            status = Status.WORKING;
            if (logSelection.get()) {
                info("End corner set: (%d, %d, %d)".formatted(end.getX(), end.getY(), end.getZ()));
            }
            BaritoneUtils.addSelection(BaritoneUtils.getSelectionManager(baritone), start, end);
            BaritoneUtils.clearArea(BaritoneUtils.getBuilderProcess(baritone), start, end);
        }
    }
    ...
    private void onRender3D(Render3DEvent event) {
        if (status == Status.SEL_START || status == Status.SEL_END) {
            ...
            event.renderer.box(result.getBlockPos(), sideColor.get(), lineColor.get(), shapeMode.get(), 0);
                ...
                && !BaritoneUtils.isBuilderActive(BaritoneUtils.getBuilderProcess(baritone))) {
            if (keepActive.get()) {
                BaritoneUtils.removeAllSelections(BaritoneUtils.getSelectionManager(baritone));
                status = Status.SEL_START;
            } else
