...
public class InfinityMiner extends Module {
    ...
    public void onActivate() {
        prevMineScanDroppedItems = (Boolean) BaritoneUtils.getSettingValue("mineScanDroppedItems");
        BaritoneUtils.setSetting("mineScanDroppedItems", true);
    ...
    public void onDeactivate() {
        BaritoneUtils.cancelEverything(BaritoneUtils.getPathingBehavior(baritone));
        BaritoneUtils.setSetting("mineScanDroppedItems", prevMineScanDroppedItems);
    }
    ...
    private void onTick(TickEvent.Post event) {
        if (isFull()) {
            if (walkHome.get()) {
                if (isBaritoneNotWalking()) {
                    info("Walking home.");
                    BaritoneUtils.setGoalAndPath(BaritoneUtils.getCustomGoalProcess(baritone), new GoalBlock(homePos));
        ...
        if (repairing) {
            ...
            if (needsRepair()) {
                ...
                repairing = true;
                BaritoneUtils.setSetting("mineScanDroppedItems", false);
                mineRepairBlocks();
                return;
            }
    ...
    private void mineTargetBlocks() {
        ...
        BaritoneUtils.cancelEverything(BaritoneUtils.getPathingBehavior(baritone));
        BaritoneUtils.mine(BaritoneUtils.getMineProcess(baritone), targetBlocks.get().toArray(array));
    }
    ...
    private void mineRepairBlocks() {
        ...
        BaritoneUtils.cancelEverything(BaritoneUtils.getPathingBehavior(baritone));
        BaritoneUtils.mine(BaritoneUtils.getMineProcess(baritone), repairBlocks.get().toArray(array));
    }
    ...
    private boolean isBaritoneNotMining() {
        Optional<Object> o = BaritoneUtils.getMostRecentInControl(BaritoneUtils.getPathingControlManager(baritone));
        return o.isPresent() && !(o.get() instanceof IMineProcess);
    }

    private boolean isBaritoneNotWalking() {
        Optional<Object> o = BaritoneUtils.getMostRecentInControl(BaritoneUtils.getPathingControlManager(baritone));
        return o.isPresent() && !(o.get() instanceof ICustomGoalProcess);
    }
