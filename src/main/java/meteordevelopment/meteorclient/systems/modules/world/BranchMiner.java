...
public class BranchMiner extends Module {
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (!baritone.getBuilderProcess().isActive()) {
            ...
            baritone.getBuilderProcess().clearArea(task.corner1, task.corner2);
        }
    }
    ...
    private void buildTasks(BlockPos baseFeet) {
        ...
        while (remaining > 0) {
            ...
            if (branchLength.get() <= 0) continue;

            BlockPos branchStart = baseFeet.offset(mainDir, offset);
            switch (branchSide.get()) {
                case Left -> tasks.addLast(makeTunnel(branchStart, rotateLeft(mainDir), tunnelHeight.get(), tunnelWidth.get(), branchLength.get()));
                case Right -> tasks.addLast(makeTunnel(branchStart, rotateRight(mainDir), tunnelHeight.get(), tunnelWidth.get(), branchLength.get()));
                case Both -> {
                    tasks.addLast(makeTunnel(branchStart, rotateLeft(mainDir), tunnelHeight.get(), tunnelWidth.get(), branchLength.get()));
                    tasks.addLast(makeTunnel(branchStart, rotateRight(mainDir), tunnelHeight.get(), tunnelWidth.get(), branchLength.get()));
                }
                case Alternate -> {
                    Direction dir = nextAlternateLeft ? rotateLeft(mainDir) : rotateRight(mainDir);
                    tasks.addLast(makeTunnel(branchStart, dir, tunnelHeight.get(), tunnelWidth.get(), branchLength.get()));
    ...
    private static ClearTask makeTunnel(BlockPos feet, Direction dir, int height, int width, int depth) {
        ...
        switch (dir) {
            case EAST -> {
                ...
                corner2 = new BlockPos(feet.getX() + depth, feet.getY() + heightOffset, feet.getZ() + widthOffset / 2 + addition);
            }
            case WEST -> {
            ...
            case NORTH -> {
                ...
                corner2 = new BlockPos(feet.getX() + widthOffset / 2 + addition, feet.getY() + heightOffset, feet.getZ() - depth);
            }
            case SOUTH -> {
        ...
        return new ClearTask(BetterBlockPos.from(corner1), BetterBlockPos.from(corner2));
    }

    private record ClearTask(BetterBlockPos corner1, BetterBlockPos corner2) {}
}
